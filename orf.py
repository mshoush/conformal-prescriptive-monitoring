...
dt_train_prefixes = read_pickle_data("./results_pred/%s/%s_dt_train_prefixes.pkl"% (dataset_name, dataset_name))
dt_test_prefixes = read_pickle_data("./results_pred/%s/%s_dt_test_prefixes.pkl" % (dataset_name, dataset_name) )
dt_val_prefixes = read_pickle_data("./results_pred/%s/%s_dt_val_prefixes.pkl" % (dataset_name, dataset_name))
...
train_data.to_pickle(os.path.join(results_dir, dataset_name + "_train_data_t_new.pkl"))
del X_train
del y_train
...
test_data.to_pickle(os.path.join(results_dir, dataset_name + "_test_data_t_new.pkl"))
...
valid_data.to_pickle(os.path.join(results_dir, dataset_name + "_valid_data_t_new.pkl"))
del X_val
del y_val

print("Read encoded data...")
df_agg = pd.read_csv('dt_transformed_agg_%s.csv'%dataset_name, sep=';')
df_static = pd.read_csv('dt_transformed_static_%s.csv'%dataset_name, sep=';')

static_agg_df = pd.concat([df_static, df_agg], axis=1)
print(static_agg_df.columns)
...
for i in itertools.product(N_trees, Min_leaf_size, Max_depth, Subsample_ratio, Lambda_reg):
    ...
    df_results.to_pickle(os.path.join(results_dir, "df_results_orf_test_new2_%s.pkl" % dataset_name))
    df_results.to_csv(os.path.join(results_dir, dataset_name + "_df_results_orf_test_new2.csv"), sep=";", index=False)
    ...
    df_results.to_pickle(os.path.join(results_dir, "df_results_orf_test_new_%s.pkl" % dataset_name))
    df_results.to_csv(os.path.join(results_dir, dataset_name+"_df_results_orf_test_new2.csv"), sep=";", index=False)
